...
public record IceModeExEvent(
     ...
     * 실려 왔습니다(예: {@code iceMode=1, freezeType=1008}). {@link #iceFlags()}는 {@code iceMode}만 분해하므로
     * 그룹 정보가 필요하면 이 메서드를 사용하세요. 매핑되지 않은 비트는 무시됩니다({@link ChatIceType.Flag#fromMask(int)} 참고).
     */
